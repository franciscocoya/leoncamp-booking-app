 ...
 * @param serviceId
 */
export const addServiceToExistingAccomodation = async (regNumber: string, serviceId: any) => {
    const { data } = await axios.post(`${API_ACCOMODATION_SERVICES}/${regNumber}/new`,
        {
            headers: {
 ...
 * @param serviceId 
 */
export const deleteServiceFromExistingAccomodation = async (regNumber: string, serviceId: number) => {
    await axios.delete(`${API_ACCOMODATION_SERVICES}/${regNumber}/delete`,
        {
            headers: {
                Authorization: `Bearer ${apiJwtToken}`
            },
